  ...
  case Msg_HiElasImLonly:
	{
     ...
     wife->GetFSM()->ChangeState(Floor::Instance());
   }
...
void Floor::Enter(MinersWife* wife)
{
  //if not already cooking put the stew in the oven
  if (!wife->Calling())
  {
    cout << "\n" << GetNameOfEntity(wife->ID()) << ":I missed you";
  
    //send a delayed message myself so that I know when to take the stew
    //out of the oven
   

    wife->SetCalling(true);
  }
}


void Floor::Execute(MinersWife* wife)
{
	switch(RandInt(0,2))
  {
  case 0:

    cout << "\n" << GetNameOfEntity(wife->ID()) << ": I love Moe";

    break;

  case 1:

    cout << "\n" << GetNameOfEntity(wife->ID()) << ": I want see Moe";

    break;

  case 2:

    cout << "\n" << GetNameOfEntity(wife->ID()) << ": I hate Bob";
    ...
    break;
	 ...
	 Dispatch->DispatchMessage(1.5,                  //time delay
                              wife->ID(),           //sender ID
							  ent_Moe,          //receiver ID
                              Msg_HiMoeImLonly,        //msg
                              NO_ADDITIONAL_INFO); 
	 wife->GetFSM()->RevertToPreviousState();
}

void Floor::Exit(MinersWife* wife)
{
  SetTextColor(FOREGROUND_GREEN|FOREGROUND_INTENSITY);
  
  cout << "\n" << GetNameOfEntity(wife->ID()) << ": Bye Let's call the next";
   ...
   Dispatch->DispatchMessage(1.5,                  //time delay
							  ...
							  Msg_CutCall,        //msg
                              NO_ADDITIONAL_INFO); 
   wife->SetCalling(false);
}


bool Floor::OnMessage(MinersWife* wife, const Telegram& msg)
{
  SetTextColor(BACKGROUND_RED|FOREGROUND_RED|FOREGROUND_GREEN|FOREGROUND_BLUE);
  ...
  switch(msg.Msg)
  {
    case Msg_CutCall:
    {
      ...
      SetTextColor(FOREGROUND_GREEN|FOREGROUND_INTENSITY);
      cout << "\n" << GetNameOfEntity(wife->ID()) << ": Now I have to stop the call";
